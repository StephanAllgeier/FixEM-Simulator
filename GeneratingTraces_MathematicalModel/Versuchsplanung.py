        ...
        for combination in all_combinations:
            ...
            for i in range(n):
                ...
                micsac_amp, intermic_duration, num_of_micsac = RandomWalkBased.RandomWalk.randomWalk(
                    ...
                    relaxation_rate=relaxation_rate, hc=hc, folderpath=folderpath, duration=10, sampling_frequency=500,
                    number_id=i)
...
if __name__ == '__main__':
    ...
    for combination in float_options:
        ...
        for i in range(n):
            ...
            relaxation_rate = combination[2]
            hc = combination[3]
            micsac_amp, intermic_duration, num_of_micsac = RandomWalkBased.RandomWalk.randomWalk(simulation_freq=simulation_rate, potential_resolution=cells_per_deg, relaxation_rate=relaxation_rate, hc = hc, duration=10, sampling_frequency=500,  number_id=i)
            comb_micsac_amp.extend(micsac_amp)
            comb_intermic_dur.extend(intermic_duration)
            comb_micsacs.extend([num_of_micsac])
        my_dict = {"MicsacAmplitudes":comb_micsac_amp, "IntermicDur":comb_intermic_dur, "NumMicsac":comb_micsacs}
        with open(fr'C:\Users\uvuik\bwSyncShare\Documents\Versuchsplanung Mathematisches Modell\AmpDurNum_simrate={str(simulation_rate)},Cells={str(cells_per_deg)},relaxationrate={str(relaxation_rate)},hc={str(hc)}.json', 'w') as fp:
