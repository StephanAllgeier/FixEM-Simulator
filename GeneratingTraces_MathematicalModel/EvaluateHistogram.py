...
def save_combination_evaluation_to_json(combinations=None, folder_to_save_to=None):
    ...
    for comb in combinations_to_eval:
        ...
        hc = comb[3]
        filename = fr'{folder_to_save_to}\AmpDurNum_simrate={str(simulation_rate)},Cells={str(cells_per_deg)},relaxationrate={str(relaxation_rate)},hc={str(hc)}_n={n}.json'
        ...
        for i in range(n):
            ...
            vel = [micsac_amp[i]/intermic_duration[i] for i in range(len(intermic_duration))]
            comb_micsac_amp.extend(micsac_amp)
            comb_intermic_dur.extend(intermic_duration)
            comb_micsacs.extend([num_of_micsac])
            comb_vel.extend(vel)
        my_dict = {"MicsacAmplitudes": comb_micsac_amp, "IntermicDur": comb_intermic_dur, "NumMicsac": comb_micsacs, "MicsacVel":comb_vel}
...
if __name__ == '__main__':
    ...
    folder_to_save_to = r"C:\Users\fanzl\bwSyncShare\Documents\Versuchsplanung Mathematisches Modell\AuswertungErgebnisse\TestOrdner GuiInput"
