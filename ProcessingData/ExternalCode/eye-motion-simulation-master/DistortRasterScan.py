...
def parse_args() -> Namespace:
    parser = ArgumentParser(
        '"scan_" is related to the input data, "disp_field_" is related to the previously simulated motion, "out_" is related to the output, everything else are parameters of the simulated raster scan.')
    ...
    if len(args.scan_dir) != 2 or not ("u" in args.scan_dir or "d" in args.scan_dir) or not (
            "l" in args.scan_dir or "r" in args.scan_dir):
    ...
    if not args.suppress_warnings and args.field_width > args.scan_field_width:
        print(
            'WARNING: field_width should usually be at most scan_field_width, preferably less to avoid extrapolation for typical eye motion')
    ...
    if not args.suppress_warnings and args.field_height > args.scan_field_height:
        print(
            'WARNING: field_height should usually be at most scan_field_height, preferably less to avoid extrapolation for typical eye motion')
    if not args.suppress_warnings and args.field_center_horz < -args.scan_field_width / 2.0 + args.field_width / 2.0 or args.field_center_horz > args.scan_field_width / 2.0 - args.field_width / 2.0:
        print(
            'WARNING: field_center_horz should usually be in the range [-scan_field_width/2.0 + field_width/2.0, scan_field_width/2.0 - field_width/2.0]')
    if not args.suppress_warnings and args.field_center_vert < -args.scan_field_height / 2.0 + args.field_height / 2.0 or args.field_center_vert > args.scan_field_width / 2.0 - args.field_height / 2.0:
        print(
            'WARNING: field_center_vert should usually be in the range [-scan_field_height/2.0 + field_height/2.0, scan_field_height/2.0 - field_height/2.0]')
...
def str2bool(val: str, argument_name: str) -> None:
    ...
    if is_true == is_false:
        die(argument_name + ' must be any value in ["' + '", "'.join(
...
x_new_d, y_new_d = np.meshgrid(x_new,
                               y_new)  # parameters are usually flipped, but the y-axis is the first iamge dimension... TODO: not 100% sure, test
...
virtual_size = [args.samples_width + args.flyback * (1 if is_xfast else 0), args.repeats,
                args.samples_height + args.flyback * (0 if is_xfast else 1)]
...
if not debug:
    ...
    if displacements.shape[0] < total_samples:
        die('Displacement vector (' + str(displacements.shape) + ') is not long enough (' + str(
            total_samples) + '). Simulate with a longer scan duration.')
...
# remove flyback
dx = dx.reshape(virtual_size)[0:args.samples_height, :,
     0:args.samples_width]  # looks weird, but the y-axis is the first image dimension...
