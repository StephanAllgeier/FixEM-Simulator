...
def get_random_directions(position_x: float, position_y: float, min_bound_xy: float, max_bound_xy: float) -> (
...
def weighted_random_selection(weights: np.ndarray, potential_weight: float) -> (np.int64, np.ndarray):
    ...
    nonzeros = (random_w < cum_weights).nonzero()[
        0]  # need to exclude equality here, otherwise, if random_w is zero, the first entry would get picked even if its weight is exact 0
...
fixation_potential = lambda offset_x, offset_y: (
            args.fixation_potential_factor * (offset_x ** 2 + offset_y ** 2) ** args.potential_norm_exponent)
