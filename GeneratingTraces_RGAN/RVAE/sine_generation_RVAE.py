...
def encoder(hidden_units_enc, emb_dim, mult):
    ...
    with tf.compat.v1.variable_scope("encoder") as scope:
        ...
        input_seq_enc = tf.compat.v1.placeholder(tf.float32, [batch_size, seq_length, num_features])

        cell = tf.compat.v1.nn.rnn_cell.LSTMCell(num_units=hidden_units_enc, state_is_tuple=True)
        enc_rnn_outputs, enc_rnn_states = tf.compat.v1.nn.dynamic_rnn(
            cell=cell,
            dtype=tf.float32,
            #sequence_length=[seq_length]*batch_size,
            inputs=input_seq_enc)

        z_mean = tf.compat.v1.layers.dense(enc_rnn_states[1], emb_dim)
        z_log_sigma_sq = tf.compat.v1.layers.dense(enc_rnn_states[1], emb_dim)

        # Draw one sample z from Gaussian distribution with mean 0 and std 1
        eps = tf.random.normal((batch_size, emb_dim), 0, 1, dtype=tf.float32)
...
def decoder(hidden_units_dec, latent_emb, input_seq_enc):
    ...
    with tf.compat.v1.variable_scope("decoder") as scope:
        ...
        W_out_dec = tf.Variable(tf.random.truncated_normal([hidden_units_dec,num_features]))
        b_out_dec = tf.Variable(tf.random.truncated_normal([num_features]))
        ...
        dec_initial_state = tf.compat.v1.layers.dense(latent_emb, hidden_units_dec, activation=tf.nn.tanh)
        ...
        cell = tf.compat.v1.nn.rnn_cell.BasicRNNCell(num_units=hidden_units_dec)
        dec_rnn_outputs, dec_rnn_states = tf.compat.v1.nn.dynamic_rnn(
            cell=cell,
            dtype=tf.float32,
            #sequence_length=[seq_length]*batch_size,
...
for config in configs:
    ...
    with tf.compat.v1.variable_scope("trainer"):
        ...
        if (optimizer_str == "adagrad_epochs") or (optimizer_str == "adagrad_minibatch_iterations"):
            train = tf.compat.v1.train.AdagradDAOptimizer(learning_rate, global_step).minimize(cost)
        elif optimizer_str == "rmsprop":
            train = tf.compat.v1.train.RMSPropOptimizer(learning_rate).minimize(cost)
