...
#        gradient_penalty = tf.reduce_mean((slopes-1)**2)
        ...
        # now for my own hack
        # sample a random h
        h = tf.random.normal(shape=X.shape, stddev=0.1)
...
def GAN_loss(Z, X, generator_settings, discriminator_settings, kappa, cond, CG, CD, CS, wrong_labels=False):
    if cond:
        ...
        # normal GAN
        G_sample = generator(Z, **generator_settings) #TODO HIER ENTSTEHT ERROR
...
def create_placeholders(batch_size, seq_length, latent_dim, num_generated_features, cond_dim):
    ...
    CG = tf.compat.v1.placeholder(tf.float32, [batch_size, cond_dim])
    CD = tf.compat.v1.placeholder(tf.float32, [batch_size, cond_dim])
    CS = tf.compat.v1.placeholder(tf.float32, [batch_size, cond_dim])
    return Z, X, CG, CD, CS
...
def generator(z, hidden_units_g, seq_length, batch_size, num_generated_features, reuse=False, parameters=None, cond_dim=0, c=None, learn_scale=True):
    ...
    Input-Variables:
        z: Latente Variablen, normalerweise zufällig generierte Werte.
        hidden_units_g: Die Anzahl der Einheiten in der LSTM-Zelle des Generators.
        seq_length: Die Länge der Sequenzen, die der Generator erzeugen soll.
        batch_size: Die Größe der Batch-Daten.
        num_generated_features: Die Anzahl der Merkmale, die der Generator für jede Zeitstufe in der Sequenz erzeugen soll.
        reuse: Ein boolischer Wert, der angibt, ob die Variablen wiederverwendet werden sollen.
        parameters: Ein optionaler Parameter, der vorberechnete Gewichtungen für den Generator enthält.
        cond_dim: Die Dimension der Bedingungen, falls das GAN bedingt ist.
        c: Die Bedingungen für den Generator, nur relevant, wenn cond_dim größer als 0 ist.
        learn_scale: Ein boolischer Wert, der angibt, ob die Skalierung trainierbar sein soll.
    """
    with tf.compat.v1.variable_scope("generator") as scope:
        if reuse:
            scope.reuse_variables()
        if parameters is None:
            W_out_G_initializer = tf.compat.v1.truncated_normal_initializer()  # Gewichtung für die Ausgabe des Generators
            b_out_G_initializer = tf.compat.v1.truncated_normal_initializer()  # Bias für die Ausgabe des Generators
        ...
        rnn_outputs, rnn_states = tf.compat.v1.nn.dynamic_rnn(
            ...
            inputs=inputs) #TODO HIER ENTSTEHT ERROR
...
#                initializer=tf.truncated_normal_initializer())
        ...
        #TODO: UserWarning: `tf.nn.rnn_cell.LSTMCell` is deprecated and will be removed in a future version. This class is equivalent as `tf.keras.layers.LSTMCell`, and will be replaced by that in Tensorflow 2.0.
...
def sample_trained_model(settings, epoch, num_samples, Z_samples=None, C_samples=None):
    ...
    Z = tf.compat.v1.placeholder(tf.float32, [num_samples, settings['seq_length'], settings['latent_dim']])
    CG = tf.compat.v1.placeholder(tf.float32, [num_samples, settings['cond_dim']])
...
def invert(settings, epoch, samples, g_tolerance=None, e_tolerance=0.1,
    ...
    # create VARIABLE Z
    Z = tf.compat.v1.get_variable(name='Z', shape=[num_samples, settings['seq_length'],
                        settings['latent_dim']],
