...
def experiment_function(settings_file=False):
    ...
    if settings_file: settings = utils.load_settings_from_file_function(settings, settings_file)
    settings['settings_file'] = settings_file
    # --- get data, split --- #
    samples, pdf, labels = data_utils.get_samples_and_labels(settings) #TODO: Woher bekomme ich die PDF Function?
    ...
    # --- build model --- #
    batch_size, seq_length, latent_dim, num_signals, cond_dim = (
        settings['batch_size'],
        settings['seq_length'],
        settings['latent_dim'],
    ...
    kappa, wrong_labels, learning_rate, l2norm_bound, batches_per_lot, dp_sigma, dp = (
        settings['kappa'],
        settings['wrong_labels'],
        settings['learning_rate'],
        settings['l2norm_bound'],
        settings['batches_per_lot'],
        settings['dp_sigma'],
        settings['dp']
    )
    ...
    eval_real_PH = tf.compat.v1.placeholder(tf.float32, [eval_eval_size, seq_length, num_generated_features])
    eval_sample_PH = tf.compat.v1.placeholder(tf.float32, [eval_eval_size, seq_length, num_generated_features])
    n_sigmas = 2
    sigma = tf.compat.v1.get_variable(name='sigma', shape=n_sigmas, initializer=tf.compat.v1.constant_initializer(
    ...
    with tf.compat.v1.variable_scope("SIGMA_optimizer"):
        sigma_solver = tf.compat.v1.train.RMSPropOptimizer(learning_rate=0.05).minimize(-that, var_list=[sigma])
    ...
    sigma_opt_vars = [var for var in tf.compat.v1.global_variables() if 'SIGMA_optimizer' in var.name]

    sess = tf.compat.v1.Session()
    sess.run(tf.compat.v1.global_variables_initializer())
    use_time, max_val, one_hot = settings['use_time'], settings['max_val'], settings['one_hot']
    ...
    #AB HIER WIRD DIE EPOCHEN TRAINIERT
    for epoch in range(num_epochs):
        ...
        if epoch % eval_freq == 0:
            ...
            for i in range(batch_multiplier):
                if CGAN:
                    eval_sample[i * batch_size:(i + 1) * batch_size, :, :] = sess.run(G_sample, feed_dict={
                        ...
                        CG: eval_C[i * batch_size:(i + 1) * batch_size]})
                else:
                    eval_sample[i * batch_size:(i + 1) * batch_size, :, :] = sess.run(G_sample, feed_dict={
                        Z: eval_Z[i * batch_size:(i + 1) * batch_size]})
            eval_sample = np.float32(eval_sample)
            eval_real = np.float32(
                samples['vali'][np.random.choice(len(samples['vali']), size=batch_multiplier * batch_size), :, :])
            ...
            sess.run(tf.compat.v1.initialize_variables(sigma_opt_vars))
            sigma_iter = 0
            that_change = sigma_opt_thresh * 2
            old_that = 0
            while that_change > sigma_opt_thresh and sigma_iter < sigma_opt_iter:
                new_sigma, that_np, _ = sess.run([sigma, that, sigma_solver], feed_dict={eval_real_PH: eval_eval_real,
                                                                                         eval_sample_PH: eval_eval_sample})
                that_change = np.abs(that_np - old_that)
                old_that = that_np
                sigma_iter += 1
            opt_sigma = sess.run(sigma)
            mmd2, that_np = sess.run(
                mix_rbf_mmd2_and_ratio(eval_test_real, eval_test_sample, biased=False, sigmas=sigma))
        ...
        ## save trace
        trace.write(
            ' '.join(map(str, [epoch, t, D_loss_curr, G_loss_curr, mmd2, that_np, pdf_sample, pdf_real])) + '\n')
        if epoch % 10 == 0:
            trace.flush()
...
if __name__ == "__main__":
    ...
    experiment_function(settings_file=r"C:\Users\uvuik\Documents\Code\RGAN-master_v2\experiments\settings\test_sine.txt")