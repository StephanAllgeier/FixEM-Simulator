...
def BuildNetwork(inputs, network_parameters):
  ...
  for conv_param in network_parameters.conv_parameters:
    ...
    conv_weights = tf.Variable(
        tf.random.truncated_normal([conv_param.patch_size,
  ...
  if network_parameters.projection_type != "NONE":
    projection = tf.Variable(tf.random.truncated_normal(
  ...
  for layer_parameters in network_parameters.layer_parameters:
    ...
    hidden_weights = tf.Variable(
        tf.random.truncated_normal([num_inputs, num_units],
...
def BatchClipByL2norm(t, upper_bound, name=None):
  ...
  with tf.compat.v1.name_scope(values=[t, upper_bound], name=name,
...
def SoftThreshold(t, threshold_ratio, name=None):
  ...
  with tf.compat.v1.name_scope(values=[t, threshold_ratio], name=name,
...
def AddGaussianNoise(t, sigma, name=None):
  ...
  with tf.compat.v1.name_scope(values=[t, sigma], name=name,
